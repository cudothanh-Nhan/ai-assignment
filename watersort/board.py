...
class Board:
    def __init__(self, glasses_list,num_of_colors):
    ...
    def get_glass_by_index(self, idx):
        return self.glassesList[idx]
    
    def is_glass_complete(self, idx):
        glass = self.get_glass_by_index(idx)
        return glass.is_full() and glass.has_single_color()
    
    def is_complete(self):
        count = 0
        for i in range(0, len(self.glasses_list)):
            if (self.is_glass_complete(self.get_glass_by_index(i))): count += 1
    ...
    def find_all_moveable_glass_by_index(self, to_idx):
        indexes = list()
        to_glass = self.get_glass_by_index(to_idx)
    ...
    def move_ball(self, move):
        ball_to_move = self.get_glass_by_index(move._from).pop_ball()
        self.get_glass_by_index(move._to).push_ball(ball_to_move)
